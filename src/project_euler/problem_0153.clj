...
(defn prime-factors [x]
  (letfn [(next-fact [n] (if (== n 2) 3 (+ n 2)))]
    (loop [x x, p 2, pfs {}]
      (cond (== 1 x)          pfs
            (> (* p p) x)     (update pfs x #(if %1 (inc %1) 1))
            (zero? (rem x p)) (recur (quot x p) p (update pfs p #(if %1 (inc %1) 1)))
            :else             (recur x (next-fact p) pfs)
            ))))

(defn count-real-gaussian-div [x]
  (->> (prime-factors x)
       vals
       (map inc)
       (reduce * 1)))
...
(defn sum-of-squares
  ([a]
   (let [bmax (int (Math/sqrt (- lim (* a a))))]
     (->> (range 1 (inc bmax))
          (map (fn [b] [a b]))
          (reduce (fn [m [a b]] (update m (+ (* a a) (* b b)) #(conj (or %1 []) [a b])))
                  {})
          )))
  ([]
   (let [amax (int (Math/sqrt lim))]
     (->> (range 1 amax)
          (map sum-of-squares)))))
