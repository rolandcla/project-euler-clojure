...
;; How many distinct arrangements of the two cubes allow for
;; all of the square numbers to be displayed?

(def digits (map #(- (int %) 48) "0123456789"))

(def valid-cubes
  (->> (range 1 10)
       (map (fn [x] (* x x)))
       (map (fn [x2] [(quot x2 10) (rem x2 10)]))
       (map (fn [[d1 d2]]
             (cond (#{6 9} d1) [[6 d2] [9 d2]]
                   (#{6 9} d2) [[d1 6] [d1 9]]
                   :else       [[d1 d2]]
                   )))
       ))

(defn ok-for-all-sq-numbers [c1 c2]
  (->> valid-cubes
       (every? (fn [vs]
                 (some (fn [[v1 v2]] (or (and (c1 v1) (c2 v2))
                                         (and (c1 v2) (c2 v1))))
                       vs)))
       ))
...
(defn solution []
  (->> (combo/cartesian-product
        (combo/combinations digits 6)
        (combo/combinations digits 6))
       (filter (fn [[c1 c2]] (ok-for-all-sq-numbers (set c1) (set c2))))
       (count)
       ((fn [cnt] (/ cnt 2)))
       ))
