...
;; they advance in a clockwise direction.
...
;; There are sixteen cards in each pile,
...
;; For this reason it should be clear that,
...
(def squares-str (str "GO A1 CC1 A2 T1 R1 B1 CH1 B2 B3 JAIL "
                      "C1 U1 C2 C3 R2 D1 CC2 D2 D3 FP "
                      "E1 CH2 E2 E3 R3 F1 F2 U2 F3 G2J "
                      "G1 G2 CC3 G3 R4 CH3 H1 T2 H2"))

(def squares (str/split squares-str #" "))

(def square-ix (zipmap squares (range)))
...
(def chance_cards
  (->> (range 16)
       (map (fn [i] (case i
                      ...
                      5 (fn [sq] "H2")
                      6 (fn [sq] "R1")
                      7 (fn [sq] (as-> (square-ix sq) <>
                                   (drop <> (cycle squares))
                                   (filter (fn [sq'] (str/starts-with? sq' "R")) <>)
                                   (first <>)))
                      8 (fn [sq] (as-> (square-ix sq) <>
                                   (drop <> (cycle squares))
                                   (filter (fn [sq'] (str/starts-with? sq' "R")) <>)
                                   (first <>)))
                      9 (fn [sq] (as-> (square-ix sq) <>
                                   (drop <> (cycle squares))
                                   (filter (fn [sq'] (str/starts-with? sq' "U")) <>)
                                   (first <>)))
                      10 (fn [sq] (nth squares (- (square-ix sq) 3)))
                      (fn [sq] sq)
                      )))
       ))
...
(defn init-state []
  {:pos        "GO"
   :cc         (reduce (fn [cs c] (conj cs c))
                       clojure.lang.PersistentQueue/EMPTY
                       (shuffle community_chest_cards))
   :ch         (reduce (fn [cs c] (conj cs c))
                       clojure.lang.PersistentQueue/EMPTY
                       (shuffle chance_cards))
   :doubles    0})

(defn go-to [state sq]
  ;;(println (state :pos) sq)
  (cond (= sq "G2J")               (recur state "JAIL")
        (str/starts-with? sq "CC") (let [c      (peek (state :cc))
                                         cs     (conj (pop (state :cc)) c)
                                         sq'    (c sq)
                                         state' (merge state {:pos sq :cc cs})]
                                     (if (= sq sq')
                                       state'
                                       (recur state' sq')))
        (str/starts-with? sq "CH") (let [c      (peek (state :ch))
                                         cs     (conj (pop (state :ch)) c)
                                         sq'    (c sq)
                                         state' (merge state {:pos sq :ch cs})]
                                     (if (= sq sq')
                                       state'
                                       (recur state' sq')))
        :true (assoc state :pos sq)
    ))

(defn advance [state [d1 d2]]
  ;;(println state d1 d2)
  (let [dbls (if (== d1 d2)
               (inc (state :doubles))
               0)]
    (if (== 3 dbls)
      (go-to (assoc state :doubles 0) "JAIL")
      (go-to (assoc state :doubles dbls) (nth (cycle squares) (+ (square-ix (state :pos)) d1 d2)))
      ))
  )

(defn add-to-stats [stats sq]
  (update stats sq (fn [x]  (inc (or x 0)))))

(defn solution []
  (->> (repeatedly 1000000 #(vector (inc (rand-int 4)) (inc (rand-int 4))))
       (reduce (fn [[stats m-state] ds]
                 [(add-to-stats stats (m-state :pos)) (advance m-state ds)])
               [{} (init-state)])
       (first)
       (sort-by second >)
       (take 3)
